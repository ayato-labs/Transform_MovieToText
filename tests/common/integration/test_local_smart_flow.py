...
class TestLocalSmartIntegration(unittest.TestCase):
    ...
    @patch("src.platforms.desktop.controllers.local_smart_ctrl.MinutesService.get_available_models")
    ...
    def test_complete_optimization_decision_flow(self, mock_specs, mock_models):
        ...
        with patch("src.platforms.desktop.controllers.local_smart_ctrl.threading.Thread") as mock_thread:
            ...
            # It should say something like "讒区・荳ｭ (Standard)" or similar
            self.assertIn("讒区・荳ｭ", self.status_text.value)
