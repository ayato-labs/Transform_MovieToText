...
@pytest.fixture
def sample_metadata():
    return {"projects": ["Ayato-AI", "Project-X", "Internal-Dev"], "categories": ["Minutes", "Strategy", "Bug-Report", "Marketing-Promotion"]}


def test_query_analyzer_reconciliation_exact_match(sample_metadata):
    # Setup Fake LLM that returns a specific project name
    fake_client = FakeLLMClient(responses={"質問:": "関係するのは Ayato-AI です。"})
...
def test_query_analyzer_synonym_match(sample_metadata):
    ...
    fake_client = FakeLLMClient(responses={"質問:": "要約を見せてください。"})
...
def test_query_analyzer_fuzzy_match(sample_metadata):
    ...
    fake_client = FakeLLMClient(responses={"質問:": "アヤトの件です。"})
...
def test_query_analyzer_no_llm_fallback(sample_metadata):
    ...
    analyzer = QueryAnalyzer(projects=sample_metadata["projects"], categories=sample_metadata["categories"], client=None)
