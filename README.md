...
- **データ永続化の局所性**: 会議データやナレッジベースは、すべて Windows 標準の AppData ディレクトリ (`%APPDATA%\TransformMovieToText`) にのみ保存され、クラウドストレージや外部APIへ漏洩することはありません。設定画面からデータフォルダをワンクリックで開くことも可能です。
