...
- **話者分離 (Speaker Diarization)** -- `sherpa-onnx` + `CAM++` により、会議における「誰が」「何を」話したかをローカルで特定。複数人の議論もクリアに記録
- **リアルタイム文字起こし** -- オンライン会議の音声をリアルタイムでキャプチャ (Windows WASAPI Loopback対応)
- **物理カット＋重ね合わせ方式** -- 5秒のオーバーラップで継ぎ目のない長時間処理を実現

### AI議事録
- 文字起こし完了後、ローカルLLM (Ollama) またはクラウドLLM (Gemini API) が自動で清書・要約・構造化
- クラウドAPIを利用する場合も、APIキーなどの機密情報はローカル (`%APPDATA%`) にのみ安全に保存されます
- 概要、決定事項、ネクストアクションを一括出力

### ナレッジ検索 (Non-Embedding RAG)
- **究極の省リソース検索**: 巨大なEmbeddingモデルを廃止し、以下の技術を組み合わせることで、VRAM消費を抑えつつ「意味的検索」に近い体験を実現しています。
    - **LLMクエリ拡張**: 検索時にLLMが「表記揺れ・類義語」を自動生成（例：「PC」→「パソコン, コンピュータ」）。
    - **SQLite FTS5 + trigram**: 3文字単位の高速全文検索インデックスにより、日本語の曖昧な部分一致も爆速で検索。
- **設計思想（なぜEmbeddingを捨てたのか）**:
    - **並列動作の担保**: ZoomやTeamsといった重いビデオ会議アプリと、AI文字起こしを「普通のノートPC」で同時に動かすためには、1MBでも多くのVRAMを空ける必要があります。
    - **LLMの推論力による代替**: 「意味の近さ」を数学的に計算するだけのEmbeddingモデルは、強力な推論能力を持つLLMがあれば代替可能です。インデックス作成時にLLMに「タグ（メタデータ）」を生成させ、検索時にはLLMに「クエリの意図と関連キーワード」を抽出させる。この**「LLM + 構造化データ検索」の組み合わせは、挙動としてベクトル検索を用いたRAGと同等、かつリソース効率において圧倒的に優れています。**
    - **メタデータへの動的変換**: 検索専用のモデルを別途常駐させるのではなく、すでにロードされているLLMにメタデータ生成を兼務させることで、推論リソースの密度を極限まで高めました。
- 過去の全会議データに対して自然言語で質問可能。
...
*   **ADR-0002: 話者分離 (Speaker Diarization) の導入 (完了)**
    *   `sherpa-onnx` と `CAM++` モデルを用いた話者分離機能を実装し、会議における「誰が」「何を」話したかを完全にローカルで特定できるようになりました。
*   **ADR-0003: PyTorch依存の完全排除とONNX化 (完了)**
    *   推論バックエンドを ONNX Runtime (`faster-whisper`, `sherpa-onnx`) に統一し、巨大な PyTorch 依存を完全に排除しました。これにより、配布サイズの劇的なスリム化と、推論の軽量化が実現しています。
...
| **話者分離** | [sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx) | PyTorchに依存せず、ONNX Runtime で高速かつ正確に話者を特定 (CAM++採用) |
| **推論エンジン** | [Ollama](https://ollama.com/) / Gemini API | 外部通信を遮断したローカル推論 (Ollama) と、高品質なクラウド推論 (Gemini API) の選択式 |
