...
### 録音データの管理について
録音した音声ファイル（MP3）は、プロジェクトルートの `recordings/` フォルダに永続保存されます。
- アプリ内の「履歴」タブから音声を別の場所に書き出す（コピー）ことができます。
- ストレージ容量を節約したい場合は、`recordings/` フォルダ内のファイルをエクスプローラー等で手動削除してください。
- データベースの履歴のみを削除する機能は現在提供していません。
