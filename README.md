...
- **データ永続化の局所性**: 会議データやナレッジベースは、すべてローカルの SQLite データベース (`history.db`) にのみ保存され、クラウドストレージや外部APIへ漏洩することはありません。
...
PythonのインストールやZIPファイルの解凍すら不要です。[Releases](https://github.com/Ayato-AI-for-Auto/Transform_MovieToText/releases) から最新の `.exe` ファイルを直接ダウンロードしてください。

**CPU版とGPU版の選び方:**
- **`TransformMovieToText_GPU.exe`**: NVIDIA製GPUを搭載したPCをお使いの場合。文字起こしやAI要約の処理速度が劇的に高速になります。
- **`TransformMovieToText_CPU.exe`**: GPU非搭載、またはNVIDIA製以外のGPUをお使いの場合。CPUのみで動作するため環境を選びません。

1. ご自身の環境に合わせて `.exe` をダウンロード。
2. [ollama.com](https://ollama.com/) から Ollama をインストール。
3. ダウンロードした `.exe` をダブルクリックで起動するだけです。（※初回起動時はファイルの展開に数十秒かかる場合があります）
