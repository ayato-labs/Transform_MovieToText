...
class RegexParser:
    ...
    def parse_raw_text(self, text):
        ...
        for word in words:
            ...
            if p_match:
                found_projects.append(p_match[0])
            t_match = difflib.get_close_matches(word, self.tags, n=1, cutoff=0.8)
            if t_match:
                found_tags.append(t_match[0])

        return {"projects": sorted(list(set(found_projects))), "categories": sorted(list(set(found_tags))), "keywords": list(set(words[:15]))}
